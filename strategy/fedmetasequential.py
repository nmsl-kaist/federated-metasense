...
class FedMetaSequential(Strategy):
    ...
    def __repr__(self) -> str:
        rep = f"FedMetaSequential(accept_failures={self.accept_failures})"
        return rep
    ...
    def configure_fit(
        ...
        if mode == "per-cond":
            ...
            if round_in_phase == 0: # First round
                ...
                # Theta is what being keep updated.
                # Index = client
                self.theta = [
                    Parameters(tensors=parameters.tensors, tensor_type=parameters.tensor_type)
                    for i in range(len(clients))
                ]
                # Initialize record for selected clients
                self.clients_record = {}
            self.clients_record[rnd] = clients
            return self.generate_fit_configuration(clients, config, round_in_phase)
        else:
            if round_in_phase == 0: # First round
                # Copy theta at this point into theta_prime
                # Index = client
                self.theta_prime = [
    ...
    def generate_fit_configuration(self, clients, config, round_in_phase):
        fit_configuration = []
        for i in range(len(clients)):
