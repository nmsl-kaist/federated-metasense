...
class FedMetaAverage2(Strategy):
    ...
    def configure_fit(
        ...
        # Sample clients
        sample_size = self.fit_clients
        if mode == "multi-cond-learn":
            sample_size = sample_size * self.multi_cond_multiplier
        clients = client_manager.sample(
            num_clients=sample_size, criterion=AvailableClientIDCriterion(self.available_fit_client_id)
        )
        ...
        if mode == "per-cond":
            ...
            if round_in_phase == 0: # First round
                ...
                # Index 1 = client / Index 2 = partition
                self.class_partitions = [
                    self.get_class_partition(self.num_classes, self.num_partitions_for_multi_cond_task)
                    for client in clients
                ]
                # Record beginning parameter at this point
                self.beginning_parameters = parameters
                # Initialize list of parameters
                # We will cumulate each theta from each training round.
                # Index 1 = client / Index 2 = partition
            ...
            for i in range(len(clients)):
                config_new = copy.deepcopy(config)
                config_new['selected-class'] = self.class_partitions[i][round_in_phase]
                config_new['theta'] = parameters_to_weights(self.beginning_parameters)
                fit_configuration.append((clients[i], FitIns(self.theta_prime[i], config_new)))
            return fit_configuration
        ...
        if mode == "per-cond":
            ...
            if round_in_cycle == self.total_rnd_per_cycle - 1: # Final round
                ...
                for theta_list in self.theta:
                    ...
                    self.final_theta.append(weights_to_parameters(aggregate(weights_results)))
                weights_results = [
                    (self._add_weights(
                        self._subtract_weights(parameters_to_weights(self.final_theta[i]), parameters_to_weights(self.theta_prime[i])),
                        parameters_to_weights(self.beginning_parameters))
                    , 1)
                    for i in range(len(self.final_theta))
                ]
                return weights_to_parameters(aggregate(weights_results)), {}
            return None, {}
