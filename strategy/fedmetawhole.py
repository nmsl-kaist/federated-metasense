...
class FedMetaWhole(Strategy):
    ...
    def __init__(
        ...
        self.total_rnd_per_cycle = num_per_cond_per_cycle + num_partitions_for_multi_cond_task
    ...
    def configure_fit(
        ...
        elif round_in_cycle < self.total_rnd_per_cycle:
            mode = "multi-cond"
        ...
        if mode == "per-cond":
            ...
            if round_in_phase == 0: # First round
                ...
                self.theta = [[] for client in clients]
            # Record selected clients
            self.clients_record = clients
            fit_configuration = []
            for i in range(len(clients)):
                config_new = copy.deepcopy(config)
                config_new['selected-class'] = self.class_partitions[i][round_in_phase]
                fit_configuration.append((clients[i], FitIns(parameters, config_new)))
    ...
    def aggregate_fit(
        ...
        if mode == "per-cond":
            ...
            results = self.match_order(results, self.clients_record)
            ...
            if round_in_cycle == self.total_rnd_per_cycle - 1: # Final round
                ...
                weights_results = [
                    (parameters_to_weights(p), 1)
                    for p in self.theta_prime
                ]
                return weights_to_parameters(aggregate(weights_results)), {}
            return None, {}
